...
async def test_sandbox_cancellation_failure_skips_finalize_and_scoring(monkeypatch) -> None:
...
async def test_harness_timeout_waits_for_failed_cancellation_and_skips_scoring(monkeypatch) -> None:
    rollout, harness, events = make_rollout(
        monkeypatch,
        block=False,
        failure_stage="none",
...
async def test_suppressed_timeout_still_checks_runtime_before_scoring(monkeypatch) -> None:
    rollout, harness, events = make_rollout(
        monkeypatch,
        block=False,
        failure_stage="none",
...
async def test_parent_cancellation_propagates_even_when_runtime_becomes_unusable(monkeypatch) -> None:
    rollout, harness, events = make_rollout(
        monkeypatch,
        block=False,
        failure_stage="none",
