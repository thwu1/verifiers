...
async def test_vmvm_runtime_stop_finishes_cleanup_under_repeated_cancellation(
    ...
    monkeypatch.setattr(vmvm, "COMMAND_CANCELLATION_GRACE_SECONDS", 5.0)
    runtime = VMVMRuntime(VMVMConfig(session_timeout=10))
    await runtime.start()
...
async def test_vmvm_runtime_cancelled_constructor_preserves_failure(
    ...
    def delayed_failed_create(config, cancel_event=None) -> FakeBackend:
        entered.set()
        assert release.wait(timeout=5)
...
async def test_vmvm_runtime_cancelled_initial_command_preserves_cleanup_failure(
    monkeypatch,
) -> None:
    backend = FakeBackend()
    ...
    def run_bash(command: str, timeout: float = 60.0) -> vmvm.VMVMBashResult:
        mkdir_started.set()
        assert mkdir_release.wait(timeout=5)
        mkdir_finished.set()
    ...
    runtime = VMVMRuntime(VMVMConfig(session_timeout=10))
    starting = asyncio.create_task(runtime.start())
    assert await asyncio.to_thread(mkdir_started.wait, 5)
    starting.cancel()

    with pytest.raises(asyncio.CancelledError) as caught:
        await starting
    assert isinstance(caught.value.__cause__, RuntimeError)
    assert "provisioning cleanup failed" in "\n".join(caught.value.__notes__)
...
async def test_vmvm_runtime_provisioning_failure_cleanup_preserves_cancellation(
    monkeypatch,
) -> None:
    backend = FakeBackend()
    backend.result = {
    ...
    def blocking_destroy() -> None:
        destroy_started.set()
        assert destroy_release.wait(timeout=5)
        backend.destroyed = True
    ...
    runtime = VMVMRuntime(VMVMConfig(session_timeout=10))
    starting = asyncio.create_task(runtime.start())
    assert await asyncio.to_thread(destroy_started.wait, 5)
